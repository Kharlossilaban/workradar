...
class _EditCategoryDialogState extends State<EditCategoryDialog> {
  ...
  void _onSave() {
    if (_formKey.currentState?.validate() ?? false) {
      // Only show confirmation for editing
      if (isEditing) {
        showDialog(
          context: context,
          builder: (ctx) => AlertDialog(
            ...
            content: Text(
              'Yakin ingin mengubah nama kategori dari "${widget.initialName}" ke "${_controller.text.trim()}"?',
            ),
            actions: [
              TextButton(
        ...
        // Create new category without confirmation
        widget.onSave(_controller.text.trim());
